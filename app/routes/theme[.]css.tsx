...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const themeColour = await themeCookie.parse(cookieHeader);
  const themeValues = getTheme(themeColour);

  const data = `
    :root {
      --color-primary: ${themeValues.colorPrimary};
      --color-primary-light: ${themeValues.colorPrimaryLight};
